...
const UserBlogs = () => {
  ...
  const sendRequest = async () => {
    try {
      ...
      console.log("API response:", res); // Debugging purpose
      return res.data; // Return only when `res` exists
    } catch (err) {
      console.error("Error in API request:", err.response?.data || err.message);
      return null; // Return null to avoid accessing properties on undefined
    }
  };
